...
def train_model_autolog(X_train, X_test, y_train, y_test):
    """Train LightGBM model with MLflow autolog - Best Configuration as Baseline"""
    print("\n" + "=" * 60)
    ...
    with mlflow.start_run(run_name="lgbm_baseline_best_config"):
        print("\nMLflow Autolog Enabled")
        print("   - Automatically logs: parameters, metrics, model, signature")
        
        # Check class distribution (should be balanced from SMOTE)
        unique, counts = np.unique(y_train, return_counts=True)
        ...
        # Initialize model with best proven parameters
        ...
        # Print metrics
        print("\nModel Performance (Best Baseline):")
        ...
        print("\nModel and metrics logged automatically via MLflow autolog")
...
def main():
    ...
    print("  Class imbalance handled (class_weight='balanced' + SMOTE)")
    ...
    print("  Model artifact saved to MLflow")
